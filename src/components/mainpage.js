...
const MainPage = () => {
    const data = [{
        notices: { // 5개 카테고리 (전체, 일반(0), 학사(1), 학생(2), 등록/장학(4))
            all: [ // 전체 6개만
                {
                ...
                {
                    title: '[졸업준비위원회] 2024학년도 2학기 마지막 졸업앨범 촬영 안내',          // 공지사항 제목
                    ...
                    <div className={styles.notice_list}>
                        {getNotices().map((notice, index) => (
                            <div key={index} className={styles.notice_item}>
                                <a href={notice.link} target="_blank" rel="noopener noreferrer" className={styles.notice_link}>
                                    <span className={styles.notice_title}>{notice.title}</span>
                                    <span className={styles.notice_date}>{formatLastModified(notice.updatedDate)}</span>
                                </a>
                            </div>
                        ))}
                    </div>
            ...
            <div className={styles.second_container}>
                    ...
                    <div className={styles.box}>
                        {myRecruitTeams.map((item, index) => (
                            <div key={index} className={styles.team}>
                                ...
                                {item.name}
                                <span className={styles.dday}>{formatLastModified(item.end_date)}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className={styles.my_page}>
                    <div className={styles.myBox}>
                        <h3>마이페이지</h3>
                        <svg width="30" height="30" viewBox="0 0 23 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7.33333 2.25H20.875M7.33333 8.5H20.875M7.33333 14.75H20.875M2.125 2.25H2.13542M2.125 8.5H2.13542M2.125 14.75H2.13542" stroke="#1E1E1E" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </div>
                    <div className={styles.buttons}>
                        <button className={styles.first_btn}>MY 모집 팀플<br />지원자 확인</button>
                        <button className={styles.second_btn}>MY 설문<br />결과보기</button>
                    </div>
                </div>
            </div>
        </div>
