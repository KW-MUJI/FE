...
const Banner = ({ isLoggedIn, handleLogout }) => {
  ...
  return (<div className={styles.banner_container}>
    <img src={logo} className={styles.logo} alt="logo" onClick={handleKWLogiClick} />
    ...
    <nav className={styles.navbar}>
      <ul className={styles.navbar_menu}>
        ...
        {isLoggedIn ? (
          <li><Link onClick={handleLogoutClick}>LOGOUT</Link></li>//로그인 상태일 때 배너

        ) : (
          <li><Link to="/login">LOGIN</Link></li> //로그아웃 상태일 때 배너
        )}
        <li><Link to="https://www.kw.ac.kr" className={styles.k_menu}>광운대학교</Link></li>
      </ul>
    </nav>
  </div>);
